defmodule UploaderT.CoreContinuous do
  use GenServer
  ...
  def handle_info({pid, {:error, :synchronize, code}}, %{watcher_pid: watcher_pid} = state) do
    IO.inspect("Error while synchronizing: #{code}")
    sync()
    {:noreply, %{watcher_pid: watcher_pid}}
  end

  def handle_info({pid, {:ok, :synchronize}}, %{watcher_pid: watcher_pid} = state) do
