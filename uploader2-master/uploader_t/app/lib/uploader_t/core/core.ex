defmodule UploaderT.Core do
  ...
  def handle_info({pid, {:error, :synchronize, code}}, state) do
    IO.inspect("Error while synchronizing: #{code}")
    # # Logger.log(:)
    {:noreply, state}
  end

  def handle_info({pid, {:ok, :synchronize}}, state) do
    IO.inspect("Synchronization completed")
    {:noreply, state}
  end

  def handle_info({pid, {:ok, :ignore}}, state) do
    {:noreply, state}
  end
  ...
  def handle_info({:retry_transmission, nil}, socket) do
    IO.inspect("No transmission to retry")
    {:noreply, socket}
  end

  def handle_info({:retry_transmission, transmission}, socket) do
    Task.Supervisor.async_nolink(MyApp.TaskSupervisor, fn -> upload(transmission, :retry) end)
  ...
  def handle_info({:delete_transmission, nil}, socket) do
    IO.inspect("No transmission to delete")
    {:noreply, socket}
  end

  def handle_info({:delete_transmission, transmission}, socket) do
    IO.inspect(transmission.file_path)
  ...
  defp upload(transmission, :retry) do
