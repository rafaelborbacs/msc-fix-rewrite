defmodule UploaderR.CoreQueue do
  use GenServer
  ...
  def handle_info({:file_event, watcher_pid, {path, events}}, %{watcher_pid: watcher_pid} = state) do
    ...
    if Enum.member?(events, :moved_to)
      and !String.contains?(path, ".gitignore")
      and String.ends_with?(path, ".zip.gpg")
        do
      transmission_uuid =
        String.split(path, "/")
        |> List.last()
  ...
  defp upload(path, transmission_uuid) do
    ...
    IO.inspect(success)
    IO.inspect(path)
    IO.inspect(decrypted_file_path)
    IO.inspect(paths_of_plain_files)
