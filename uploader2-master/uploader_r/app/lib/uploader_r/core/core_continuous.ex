defmodule UploaderR.CoreContinuous do
  use GenServer
  ...
  def handle_info({pid, {:error, :store, code}}, %{watcher_pid: watcher_pid} = state) do
    IO.inspect("Error while synchronizing: #{code}")
    store()
    {:noreply, %{watcher_pid: watcher_pid}}
  end

  def handle_info({pid, {:ok, :store}}, %{watcher_pid: watcher_pid} = state) do
  ...
  def store() do
    ...
    Task.Supervisor.async_nolink(MyApp.TaskSupervisor, fn ->
      ...
      {output, exit_code} =  System.shell(
        ~s(storescu -c "#{repository_aetitle}@#{repository_host}:#{repository_port}" -b "#{self_ae_title}" /app/decompressed/ 2>&1 >/dev/null)
      )

      IO.inspect("Errors:")
      IO.inspect(output)
      IO.inspect(length(output))
      File.write!("/app/errors.txt", output <> "\n----------------------------------------------------------------------------------------------------------------------\n", [:append])
