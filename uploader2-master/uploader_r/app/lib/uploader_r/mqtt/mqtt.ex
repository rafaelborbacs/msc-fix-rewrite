defmodule UploaderR.MQTT do
  use GenServer
  alias UploaderR.SSH
  alias UploaderR.Logger

  # ~r|G/\w+/T/\w+/R/\w+|

  @topic_regexes [
    # Connect commands from the Manager Unit(G)
    {~r|G/connect/T/\w+/R/\w+|, :connect},
    {~r|G/disconnect/T/\w+/R/\w+|, :disconnect}
  ]
  ...
  def init(args) do
    ...
    System.cmd("service", ["ssh", "restart"])
  ...
  def do_handle({:connect, literal}, payload) do
    # get the target unit(R) identifier from literal
    # Keep in mind the literal is on the form:
    # "G/connect/T/:origin_unit.public_key_identifier/R/:target_unit_identifier"
    # Split the literal on the "/" character
    target_unit_identifier =
    ...
    if target_unit_identifier == SSH.identifier(:self) do
      # Save the origin unit(T) public key contained in the payload
      SSH.save_public_key(payload)
    end
  end

  def do_handle({:disconnect, literal}, payload) do
    # get the target unit(R) identifier from literal
    # Keep in mind the literal is on the form:
    # "G/connect/T/:origin_unit.public_key_identifier/R/:target_unit_identifier"
    # Split the literal on the "/" character
    ...
    # If the target unit is this unit(R)
    if target_unit_identifier == SSH.identifier(:self) do
      # Delete the origin unit(T) public key contained in the payload
      SSH.delete_public_key(payload)
    end
  end
